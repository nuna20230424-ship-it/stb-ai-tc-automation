...
### 신규: `docs/automation-prerequisites.md`
- **A. TC 문서 필수 컬럼** 7종 + 한국어 컬럼명 override 예시
- **B. 자동 실행 하드웨어** 7종 (Reference STB / DUT / 캡처카드 / IR / UART / Smart Plug / 운영 노트북)
- **C. 자격증명 매트릭스** 5종 (precondition별 자동 매칭)
- **D. AI/LLM** 옵션 정리 (외부 API / Ollama 로컬 / 임베딩)
- **E. 베이스라인** seed 명령어 + 6~8시간 작업 안내
- **F. 자동화 적합성** ✅/❌ 분류 + manual_only 태그 정책
- 🎯 우선순위 + 📋 빠른 체크리스트 + 업로드 한 줄 명령어

### README.md
- "TC 자동화 필수 요소" + "엑셀 업로드 절차" 카드 추가

### 사용자 피드백 반영
> "tc를 업로드해서 자동화를 만들때 필요한 필수적인 요소에 대해 알려줘"

대화에서 정리한 내용을 영구 문서화 — 다음 주 집에서 엑셀 작업 시 참조용.
...
전면 정리. 다음 주 화요일(2026-06-02) private 전환 전 사전 정리.

### 삭제 / 정리 대상
- ❌ **삭제**: `docs/36-kt-skb-case-study.md` (입찰 케이스 스터디 전체)
- ✏️ **참조 제거**: README.md (목차 카드 1줄)
- ✏️ **참조 제거**: docs/email-report.md (메일 본문 — 차별점·일정·첨부 URL 3곳)
- ✏️ **참조 제거**: docs/console.html (업로드 placeholder + 데모 mock 라벨 + "고객사 입찰 데모 영상" 문구)
- ✏️ **참조 제거**: docs/23-scale-300-500-tc-strategy.md (4곳: 시장 분석 / Phase 5 항목 / 정보 한계 / 차별화)
- ✏️ **참조 제거**: docs/35-rdk-fallback.md (RDK 채용 사업자 표현으로 일반화)
- ✏️ **참조 제거**: docs/specs/example-disney-plus.md (배포 스펙 표현 일반화)
- ✏️ **참조 제거**: infrastructure/notebook-gateway/services/incident-mcp/main.py (docstring 1줄)
- ✏️ **CHANGELOG 자체**: 5곳 (업데이트 25 / 39 / 46 / 48 / 영상 분석 본문)

### 사용자 피드백 반영
> "kt skb 입찰내용은 삭제해줘"

### 미적용 (의도적)
- Git 이력 rewrite (BFG/filter-repo) — 미적용. 다음 주 private 전환 시 외부 노출 차단됨.
  필요 시 별도 작업.
...
영상 분석을 **end-to-end 자동화** — 운영팀은 영상만 던지면 됨. 사내 시연에서 "영상 →
타임라인 + 썸네일 + JIRA 자동 등록" 30초 데모 가능.
...
(사내 시연 영상 / 사용자 제보 / 야간 운영 녹화)을 시나리오 없이 즉시 분석할 수 있는
...
기존엔 없었음 → 이번 업데이트로 free-form 영상 분석 기능 추가. 영상만 받으면
...
## 2026-05-27 (업데이트 39) — Phase 5: State Graph + RDK 폴백
...
- docs/34-state-graph-navigation.md, docs/35-rdk-fallback.md
...
- 국내 시그널: STB QA 공개 사례 부재 → Kaon 차별화 여백.
  업계 4K UHD 시험 절차(214채/60채/20초 자핑)에서 표준 시험 패턴 식별
- AI 기법 진위 판정:
